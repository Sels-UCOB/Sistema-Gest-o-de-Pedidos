...
import type { AcertoBolsa } from "@/lib/campanhas/types/bolsa";
...
export function DocumentoBolsasPDF({ acertoNome, bolsas, extraHeaders }: Props) {
  ...
  // Colunas visíveis: apenas headers não-vazios, mantendo o índice original para alinhar com colunas
    ...
    <Document>
      <Page style={s.page} orientation="landscape" size="A4">
        ...
        <View style={{ marginBottom: 12 }}>
          <Text style={s.titulo}>Colportores que bateram a bolsa</Text>
          <Text style={s.subtitulo}>
            {acertoNome} · {bolsas.length} colportor{bolsas.length !== 1 ? "es" : ""} · Gerado em {dataGeracao}
          </Text>
        </View>

        {bolsas.length === 0 ? (
          <View style={s.emptyBox}>
            <Text style={s.emptyText}>Nenhuma bolsa registrada.</Text>
          </View>
        ) : (
          <View>
            {/* Cabeçalho da tabela */}
        ...
        <View style={s.rodape} fixed>
          <Text style={s.rodapeTexto}>Sistema SELS — Bolsas</Text>
          <Text
            style={s.rodapeTexto}
