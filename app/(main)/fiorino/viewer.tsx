...
const ROT_STEP = Math.PI / 10; // ~18° per click
...
export default function FiorinoViewer({ boxes }: Props) {
  ...
  useEffect(() => {
    ...
    // Mouse events
    const onMD = (e: MouseEvent) => { drag.current.active = true; drag.current.prevX = e.clientX; drag.current.prevY = e.clientY; };
    const onMM = (e: MouseEvent) => {
      if (!drag.current.active) return;
      applyDelta((e.clientX - drag.current.prevX) * 0.013, (e.clientY - drag.current.prevY) * 0.013);
      drag.current.prevX = e.clientX; drag.current.prevY = e.clientY;
    };
    const onMU = () => { drag.current.active = false; };

    // Touch events
    const onTS = (e: TouchEvent) => { drag.current.active = true; drag.current.prevX = e.touches[0].clientX; drag.current.prevY = e.touches[0].clientY; };
    const onTM = (e: TouchEvent) => {
      if (!drag.current.active) return;
      applyDelta((e.touches[0].clientX - drag.current.prevX) * 0.013, (e.touches[0].clientY - drag.current.prevY) * 0.013);
      drag.current.prevX = e.touches[0].clientX; drag.current.prevY = e.touches[0].clientY;
    };
    const onTE = () => { drag.current.active = false; };
    ...
    sceneRef.current = { renderer, scene, camera, pivot, boxGroup, frameId };
  ...
  return (
    <div className="relative w-full h-full select-none">
      ...
      {/* Rotation controls — cross layout, bottom-right */}
      <div className="absolute bottom-3 right-3 flex flex-col items-center gap-1">
        <CtrlBtn onClick={() => applyDelta(0, -ROT_STEP)}>
          <ChevronUp className="w-4 h-4" />
        </CtrlBtn>
        <div className="flex items-center gap-1">
          <CtrlBtn onClick={() => applyDelta(-ROT_STEP, 0)}>
            <ChevronLeft className="w-4 h-4" />
          </CtrlBtn>
          <CtrlBtn onClick={resetView}>
